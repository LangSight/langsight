...
**Do not set `LANGSIGHT_WORKERS` above `1`.** The API enforces this at startup:
...
The in-memory rate limiter (slowapi), SSE live-feed broadcaster, and auth token cache are all in-process state. Running N workers multiplies the effective rate limit by N, which breaks brute-force protection on the login endpoint and allows SSE events to be missed by clients connected to a different worker.

Horizontal scaling requires a Redis-backed rate limiter. Until that is shipped, keep `LANGSIGHT_WORKERS=1` and scale vertically (increase CPU/memory) if needed.
...
server {
    ...
    # SSE live feed — disable buffering so events arrive in real time
    location /api/live/ {
...
<Note>
  The Next.js dashboard already proxies `/api/*` calls to FastAPI internally. Nginx only needs to proxy to port 3003 — do not create a separate Nginx `location /api/` block that points to port 8000, or you will bypass the dashboard's auth proxy layer.
</Note>
