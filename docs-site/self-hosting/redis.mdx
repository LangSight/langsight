...
This will:
1. Add `REDIS_PASSWORD` and `LANGSIGHT_REDIS_URL` to `.env` (if not already present)
2. Start Redis alongside the core stack using `--profile redis`
3. Wait for all 5 services (postgres, clickhouse, api, dashboard, redis) to be healthy
4. Print confirmation in the success output
