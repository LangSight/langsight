...
| *(API workers)* | `LANGSIGHT_WORKERS` | Number of Uvicorn worker processes (default: `1`). **Must stay at `1`.** See warning below. |
...
<Warning>
  **`LANGSIGHT_WORKERS` must stay at `1`.** Setting it higher causes a startup `RuntimeError` that prevents the API from starting:

  ```
  RuntimeError: LANGSIGHT_WORKERS=4 is not safe with the current in-memory rate
  limiter. Each worker gets its own independent counter, so the effective rate
  limit becomes limit×4. Set LANGSIGHT_WORKERS=1 (the default), or replace
  slowapi with a Redis-backed limiter before enabling multi-worker mode.
  ```

  The rate limiter (slowapi), SSE live-feed broadcaster, and auth cache are all in-process Python state. Under multi-process Uvicorn each worker holds an independent copy: brute-force protection on `/api/users/verify` becomes ineffective, SSE events published in one worker are not delivered to clients connected to another, and auth cache state diverges.

  To safely run multiple workers you must replace slowapi with a Redis-backed limiter. Until that is shipped, `LANGSIGHT_WORKERS=1` is enforced at startup.
</Warning>
