---
title: "Introduction"
description: "LangSight is the observability platform for AI agent actions — full traces of every tool call across single and multi-agent workflows, with deep MCP health monitoring and security scanning built in."
---

## What is LangSight?

LangSight answers the question every on-call engineer asks first: "What did my agent call, in what order, how long did each tool take, which ones failed, and what did it cost?" Langfuse and LangSmith show what your agent *thought* — prompts, completions, evals. LangSight shows what your agent *did* — every tool action, in sequence, with latency, errors, and cost attribution.
...
<CardGroup cols={2}>
  ...
  <Card title="Multi-Agent Call Trees" icon="diagram-project">
    When Agent A hands off to Agent B which calls Agent C, trace the full tree. Parent-child span relationships (same model as OpenTelemetry distributed tracing).
  </Card>
  <Card title="MCP Health Monitoring" icon="heart-pulse">
    For MCP servers specifically: proactive availability checks, latency tracking, and schema drift detection. Synthetic probes, not just passive recording.
  </Card>
  <Card title="MCP Security Scanning" icon="shield-check">
    For MCP servers specifically: CVE detection, OWASP MCP Top 10 audit, tool poisoning detection, and auth gap analysis.
  </Card>
</CardGroup>
