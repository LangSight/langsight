...
### Recommended pattern (v1.0 helpers)

Use `create_handoff()` and `wrap_child_agent()` for the simplest multi-agent setup. These helpers emit a properly linked handoff span and pre-configure the child proxy's parent references.
...
async def orchestrator_agent():
    ...
    # create_handoff emits the span and returns it
    ...
    traced_confluence = client.wrap_child_agent(
...
For cases where you need more control over span construction, you can create handoff spans directly.
