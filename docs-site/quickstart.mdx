...
<Tabs>
  <Tab title="Full dashboard (recommended)">
    ...
    Open **http://localhost:3003** and log in with the admin credentials
    printed by the script (also saved in `.env`). You'll see an empty
    dashboard with a "Default" project — instrument your first agent below
    to start seeing traces.

    <Note>
      The quickstart script generates all required secrets, starts Docker
      Compose, and creates your admin account automatically. Check `.env`
      for the generated credentials if you need them later.
    </Note>
  </Tab>
