...
FastAPI trusts `X-User-Id` / `X-User-Role` **only from localhost** (`127.0.0.1` or `::1`). External clients that set these headers are ignored — the headers have no effect when the request does not originate from the proxy.
...
<Note>
  The audit log is stored in an in-memory ring buffer (last 50 events). It resets on API restart. Persistent audit logging will be added in a future release.
</Note>
