...
In the **MCP Servers** detail panel, the **Health** tab shows a **Schema Drift** sub-section listing all historical drift events with their classifications. Click any event to expand the full structural diff.

The **Consumers** tab (visible when a server has an active `BREAKING` drift) shows the consumer impact table — the same data as `langsight drift-impact`, updated in real time.
