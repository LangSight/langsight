...
**Health tool probe failure** — if you configure a `health_tool` (see [Backend probe](#backend-probe) below), and that tool call fails after a successful `tools/list`, the server is marked `degraded`. This means the MCP layer is reachable but the backend application it wraps is down or unhealthy.
