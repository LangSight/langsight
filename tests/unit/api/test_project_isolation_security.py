...
# ===========================================================================
# 3. Cross-project session comparison
# ===========================================================================

class TestSessionComparisonProjectIsolation:
    """Invariant: compare_sessions must pass project_id to storage so that a
    session from a foreign project returns empty spans.  The API endpoint then
    raises 404 — it must NOT return foreign span data, even partially.

    Two enforcement layers are tested:
      a) Storage-level: compare_sessions receives the project_id and each
         get_session_trace call filters by it.
      b) Post-hoc router filter: spans whose project_id does not match the
         caller's project are stripped; if all spans for a session are stripped
         the response is 404.
    """

    async def test_compare_passes_project_id_to_storage(self, open_client) -> None:
        """project_id must be forwarded to storage.compare_sessions, not dropped."""
        c, storage = open_client
        storage.compare_sessions = AsyncMock(return_value={
            "session_a": "sess-a",
            "session_b": "sess-b",
            "spans_a": [_make_span("sess-a", "project-x")],
            "spans_b": [_make_span("sess-b", "project-x")],
            "diff": [],
            "summary": {"matched": 1, "diverged": 0, "only_a": 0, "only_b": 0},
        })

        await c.get("/api/agents/sessions/compare?a=sess-a&b=sess-b&project_id=project-x")

        storage.compare_sessions.assert_called_once_with(
            "sess-a", "sess-b", project_id="project-x"
        )

    async def test_foreign_session_b_returns_404(self, open_client) -> None:
        """session_b belongs to project-y; caller is scoped to project-x.

        Storage returns spans_b with project_id='project-y'.  The post-hoc
        filter in the router strips them.  Empty spans_b with non-empty
        original spans_b triggers 404 — the caller cannot see the foreign data.
        """
        c, storage = open_client
        storage.compare_sessions = AsyncMock(return_value={
            "session_a": "sess-a",
            "session_b": "sess-foreign",
            "spans_a": [_make_span("sess-a", "project-x")],
            # spans_b has data but it belongs to project-y, not project-x
            "spans_b": [_make_span("sess-foreign", "project-y")],
            "diff": [],
            "summary": {"matched": 0, "diverged": 0, "only_a": 1, "only_b": 1},
        })

        response = await c.get(
            "/api/agents/sessions/compare?a=sess-a&b=sess-foreign&project_id=project-x"
        )

        assert response.status_code == 404, (
            f"Expected 404 when session_b is in a foreign project, got {response.status_code}"
        )

    async def test_foreign_session_a_returns_404(self, open_client) -> None:
        """session_a belongs to project-y; caller is project-x.  Must be 404."""
        c, storage = open_client
        storage.compare_sessions = AsyncMock(return_value={
            "session_a": "sess-foreign",
            "session_b": "sess-b",
            "spans_a": [_make_span("sess-foreign", "project-y")],
            "spans_b": [_make_span("sess-b", "project-x")],
            "diff": [],
            "summary": {"matched": 0, "diverged": 0, "only_a": 1, "only_b": 1},
        })
        ...
        response = await c.get(
            "/api/agents/sessions/compare?a=sess-foreign&b=sess-b&project_id=project-x"
        )

        assert response.status_code == 404, (
            f"Expected 404 when session_a is in a foreign project, got {response.status_code}"
        )

    async def test_both_sessions_in_correct_project_returns_200(
        self, open_client
    ) -> None:
        """Baseline: both sessions belong to the caller's project → 200."""
        c, storage = open_client
        storage.compare_sessions = AsyncMock(return_value={
            "session_a": "sess-a",
            "session_b": "sess-b",
        ...
        response = await c.get(
            "/api/agents/sessions/compare?a=sess-a&b=sess-b&project_id=project-x"
        )
        ...
        assert len(data["spans_a"]) == 1
        assert len(data["spans_b"]) == 1

    async def test_spans_with_null_project_id_are_not_filtered_out(
        self, open_client
    ) -> None:
        """Spans ingested before project tagging (project_id=None) must not be
        rejected — they are treated as belonging to any project."""
        c, storage = open_client
        span_without_project = _make_span("sess-legacy", "project-x")
        span_without_project["project_id"] = None  # pre-tagging span

        storage.compare_sessions = AsyncMock(return_value={
            "session_a": "sess-legacy",
            "session_b": "sess-b",
            "spans_a": [span_without_project],
            "spans_b": [_make_span("sess-b", "project-x")],
            "diff": [],
            "summary": {"matched": 1, "diverged": 0, "only_a": 0, "only_b": 0},
        })
        ...
        response = await c.get(
            "/api/agents/sessions/compare?a=sess-legacy&b=sess-b&project_id=project-x"
        )

        assert response.status_code == 200, (
            "Legacy spans with project_id=None must not cause a 404"
        )

    async def test_compare_without_project_id_does_not_filter_spans(
        self, open_client
    ) -> None:
        """Admin path (no project_id) bypasses the post-hoc filter entirely."""
        c, storage = open_client
        storage.compare_sessions = AsyncMock(return_value={
            "session_a": "sess-a",
            "session_b": "sess-b",
            "spans_a": [_make_span("sess-a", "project-x")],
            "spans_b": [_make_span("sess-b", "project-y")],
            "diff": [],
            "summary": {"matched": 0, "diverged": 0, "only_a": 1, "only_b": 1},
        })

        response = await c.get("/api/agents/sessions/compare?a=sess-a&b=sess-b")

        # Admin sees all projects — no filtering, both spans present
        assert response.status_code == 200
        data = response.json()
        assert len(data["spans_a"]) == 1
        assert len(data["spans_b"]) == 1
