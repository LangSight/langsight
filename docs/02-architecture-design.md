...
- **MCP auto-patch via `_patch_mcp()`** (added v0.12.0, 2026-04-01): `auto_patch()` now calls `_patch_mcp()`, which monkey-patches `mcp.ClientSession.call_tool` at the class level. Every MCP tool call after `auto_patch()` is automatically traced with the correct `agent_name`, `session_id`, and `trace_id` from the active `session()` context. The original method is preserved in `_originals["mcp_call_tool"]` and restored by `unpatch()`. `mcp` is now included in the `patched`/`skipped_missing` lists in the `auto_patch.complete` structured log event. This decision was taken because `wrap()` required an explicit MCP session reference — not available in all framework integrations. Class-level patching means any `mcp.ClientSession` instance (including those created inside framework internals) is automatically traced.
- **Handoff auto-detection via `_HANDOFF_TOOL_RE`** (added v0.12.0, 2026-04-01): `_maybe_emit_handoffs()` in `llm_wrapper.py` inspects every `llm_intent` span after LLM generation. Tool names matching `call_*`, `delegate_*`, `invoke_*`, `transfer_to_*`, `run_*`, or `dispatch_*` trigger automatic handoff span emission to the target agent (captured group). This was motivated by the `gemini-sdk-ai-e2e` pattern where agents are called via `call_analyst`/`call_procurement` tools — the previous approach required manual `create_handoff()` after every LLM call. Self-handoffs (target == source) are suppressed. The pattern is intentionally broad; false positives are rare because tool names like `run_sql` don't match the naming convention for inter-agent delegation in practice.
- **Contextvar fallback in `wrap()` and `wrap_llm()`** (added v0.12.0, 2026-04-01): Both methods now read `_agent_ctx`, `_session_ctx`, and `_trace_ctx` as fallback when their corresponding params are not explicitly provided. This is the completion of the contextvar-first design: `session()` sets the context, all downstream calls inherit it. The prior requirement to pass `agent_name`/`session_id` to every `wrap()` call was the primary friction point for adoption.
