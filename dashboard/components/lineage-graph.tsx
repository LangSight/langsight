                    ...
                    >
                      ...
                      {hasMet && (
                        <div className="flex items-center flex-wrap gap-1.5 mt-1.5">
                          ...
                          {node.meta?.input_tokens != null && Number(node.meta.input_tokens) > 0 && <span className="text-[8px] px-1.5 py-[2px] rounded-full" style={{ background: "hsl(var(--muted))", color: "hsl(var(--muted-foreground))", fontFamily: "var(--font-geist-mono)" }}>{Number(node.meta.input_tokens).toLocaleString()}→{Number(node.meta.output_tokens ?? 0).toLocaleString()}{Number(node.meta.thinking_tokens ?? 0) > 0 ? `+${Number(node.meta.thinking_tokens).toLocaleString()}t` : ""}</span>}
